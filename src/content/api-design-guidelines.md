...
🛑 **DO NOT** accept a body for a `GET` or `DELETE` request. According to `HTTP/1.1` specifications, `GET` and `DELETE` requests should not have a request body. Both `GET` and `DELETE` methods are considered "safe" methods, meaning they should not have any significant side effects on the server's resources or data. As a result, including a request body for `GET` or `DELET`E requests goes against the intended semantics of these `HTTP` methods.

🛑 **DO NOT** use special characters in paths. The use of special characters in paths can lead to issues with parsing, encoding, and security, and it may cause confusion for both API consumers and developers.
